...
func NewRedisUserRepository(redisClient *redis.Client, next repository.UserRepository, log *slog.Logger) repository.UserRepository {
	return &redisUserRepository{
		redisClient: redisClient,
		next:        next,
		log:         log.With("repo", "redis"),
	}
}
...
func (r *redisUserRepository) Save(ctx context.Context, user *model.User) error {
	...
	if user.ID > 0 {
		...
		if err := r.redisClient.Del(ctx, keyID).Err(); err != nil {
			r.log.Warn("failed to delete user cache by id", "key", keyID, "error", err)
		} else {
			r.log.Debug("user cache deleted by id", "key", keyID)
		}
	}
	if user.Username != "" {
		keyUsername := fmt.Sprintf(userCacheKeyByUsername, user.Username)
		if err := r.redisClient.Del(ctx, keyUsername).Err(); err != nil {
			r.log.Warn("failed to delete user cache by username", "key", keyUsername, "error", err)
		} else {
			r.log.Debug("user cache deleted by username", "key", keyUsername)
		}
	}
...
func (r *redisUserRepository) findUserFromCacheOrNext(ctx context.Context, key string, findNext func(context.Context) (*model.User, error)) (*model.User, error) {
	...
	if err == nil {
		...
		if json.Unmarshal([]byte(val), &user) == nil {
			r.log.Debug("user cache hit", "key", key)
			return &user, nil
		}
	...
	if err != nil {
		r.log.Warn("failed to marshal user for cache", "user_id", user.ID, "error", err)
	} else {
		if err := r.redisClient.Set(ctx, key, jsonData, userCacheDuration).Err(); err != nil {
			r.log.Warn("failed to set user cache", "key", key, "error", err)
		} else {
			r.log.Debug("user cache set", "key", key)
		}
	}
